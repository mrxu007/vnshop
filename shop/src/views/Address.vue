<template>
  <div class="Address">
    <Header />
    <NavBread><span>支付订单流程</span></NavBread>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="lemall-msg-info hidden">
            <span
              >The region you selected is not within our delivery area. Please
              select another shipping address within our delivery areas.</span
            >
          </div>
          ...
          <div class="next-btn-wrap">
            <router-link
              ...
              :to="{
                path: '/orderConfirm',
                query: { addressId: checkedAddressId }
              }"
              >下一步</router-link
            >
          </div>
...
export default {
  ...
  methods: {
    //获取收货地址
    getAddress() {
      this.$http.post("/users/addressList").then(res => {
        this.addressList = res.data.result;
        //获取收获默认地址id
        this.addressList.forEach(item => {
          if (item.isDefault == true) {
            this.checkedAddressId = item.addressId;
          }
        });
      });
    },
